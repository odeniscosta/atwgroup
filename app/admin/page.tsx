...
export default async function AdminPage() {
  ...
  return <><div className="flex justify-end bg-[#f8f4ee] px-4 pt-5 md:px-8">{user.role === "ADMIN" && <Link href="/admin/configuracoes" className="inline-flex items-center rounded-full border border-[#ded6cd] bg-white px-5 py-2.5 text-sm font-black text-[#4d443d] hover:border-[#f26822]">Configurações do Mercado Pago</Link>}</div><OperationsDashboard role={user.role} /></>;
}
