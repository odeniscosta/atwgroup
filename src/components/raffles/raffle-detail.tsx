...
export function RaffleDetail({ raffle }: Readonly<{ raffle: RafflePublic }>) {
  ...
  const primaryImage = raffle.images[0]?.url ?? raffle.imageUrl;
  ...
  return (
    <main className="container-shell py-8 md:py-12">
      ...
      <div className="mt-7 grid gap-8 lg:grid-cols-[1fr_390px]">
        <section>
          <div className="relative overflow-hidden rounded-3xl bg-[radial-gradient(circle_at_20%_20%,#ffb77f,transparent_42%),linear-gradient(135deg,#f26822,#17120f)] p-7 text-white md:p-10"><div className="absolute inset-0 bg-black/20" />{primaryImage && <Image src={primaryImage} alt={raffle.images[0]?.alt ?? raffle.title} fill sizes="(max-width: 1024px) 100vw, 66vw" className="object-cover" />}<div className="relative"><Ticket size={50} strokeWidth={1.4} /><p className="mt-7 text-xs font-black uppercase tracking-[0.18em] text-[#ffd0b5]">Rifa ATW Group</p><h1 className="mt-3 text-4xl font-black tracking-[-0.07em] md:text-6xl">{raffle.title}</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-white/75">{raffle.description ?? "Escolha seus números e participe."}</p></div></div>
          {raffle.images.length > 1 && <div className="mt-3 grid grid-cols-5 gap-2">{raffle.images.map((image) => <div key={image.id} className="relative aspect-square overflow-hidden rounded-xl bg-[#f7f2ec]"><Image src={image.url} alt={image.alt ?? raffle.title} fill sizes="120px" className="object-cover" /></div>)}</div>}
